...
async def handle_webapp_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Check if we have web app data
        if update.message and update.message.web_app_data:
            web_app_data = update.message.web_app_data
            logger.info(f"Received web app data: {web_app_data.data}")
            ...
            logger.error("No web app data received")
            await context.bot.send_message(
                chat_id=update.effective_chat.id,
        ...
        logger.error(f"Unexpected error in handle_webapp_data: {e}")
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
...
application.add_handler(MessageHandler(filters.StatusUpdate.WEB_APP_DATA | filters.Message, handle_webapp_data))
