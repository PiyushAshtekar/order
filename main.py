...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=chat_id, text="🍔 Welcome to Raju Burger!\nLet's get started! 🎉"
    )
    ...
    try:
        keyboard = [
            [
                InlineKeyboardButton(
                    "🛒 Order Food", web_app=WebAppInfo(url=f"{WEBHOOK_URL}/menu")
                )
            ]
        ]
        ...
        await update.message.reply_text(
            "Please tap below to order your perfect lunch! 🍽️", reply_markup=reply_markup
        )
...
async def telegram_webhook():
    try:
        ...
        logger.info(f"Full Update object: {update.to_dict()}")  # Log the full update!
...
async def post_init(app: ApplicationBuilder):
    ...
    try:
        ...
        logger.info(
            f"Webhook status: URL={webhook_info.url}, Pending updates: {webhook_info.pending_update_count}"
        )

        if webhook_info.url != webhook_url:
            logger.error(
                f"Webhook URL mismatch! Expected: {webhook_url}, Got: {webhook_info.url}"
            )
        else:
...
async def handle_webapp_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logger.info("--- SIMPLIFIED handle_webapp_data CALLED ---")
    logger.info(f"Full Update RECEIVED in handle_webapp_data: {update.to_dict()}")
    try:
        await context.bot.send_message(
            chat_id=update.effective_chat.id, text="Web App data received (testing)!"
        )
    except Exception as e:
